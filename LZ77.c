...
int main(int argc, char *argv[]) { 
    if(argc != 3) { /* Checks if the user properly used this program */
        printf("Incorrect Usage!\nHere's how it should look: %s INPUT.txt OUTPUT.lz77\n", argv[0]);
        return 1;
    }
    
    FILE *in = fopen(argv[1], "rb");                                    /* "rb" is read binary */
    if(in == NULL) {                                                    /* Checks if the file was opened properly */
        perror("Input file failed to open.");   
        return 1;
    }

    FILE *out = fopen(argv[2], "wb");                                   /* "wb" is write binary */
    if(out == NULL) {                                                   /* Checks if the file was opened properly */
        perror("Output file failed to open.");
        fclose(in);                                                     /* Closes the first file if the second one doesn't open to help prevent errors in the future */
        return 1;
    }
    
    compressLZ77(in, out);                                              /* This passes the two files through the compressor */ 

    if(fclose(out) == EOF) {                                            /* Closes the output file and checks to see if the file closed properly. If the file did not close properly it will return a -1, which is equivalent to EOF */
        perror("Output file failed to close.");
    }
    if(fclose(in) == EOF) {                                             /* Closes the input file and checks to see if the file closed properly. If the file did not close properly, it will return a -1, which is equivalent to EOF */
